...
def abs_loss(generator, symbols, vocab_size, device, train=True, label_smoothing=0.0, copy_generator=None):
    compute = NMTLossCompute(
        generator, symbols, vocab_size,
        label_smoothing=label_smoothing if train else 0.0, copy_generator=copy_generator)
    compute.to(device)
    return compute
...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, batch, output, output_copy, copy_state, y_emb, z_vec):
        """
        ...
        """
        shard_state = self._make_shard_state(batch, output, output_copy, copy_state, y_emb, z_vec)
        _, batch_stats = self._compute_loss(batch, **shard_state)

        return batch_stats

    def sharded_compute_loss(self, batch, output, output_copy, copy_state, y_emb, z_vec,
                              shard_size,
                             normalization):
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, generator, symbols, vocab_size,
                 label_smoothing=0.0, copy_generator=None):
    ...
    def _make_shard_state(self, batch, output, output_copy, copy_state, y_emb, z_vec):
        if not COPY:
            output_copy = 1
        return {
            "output": output,
    ...
    def _compute_loss(self, batch, output, output_copy, copy_state, y_emb, z_vec, target):
